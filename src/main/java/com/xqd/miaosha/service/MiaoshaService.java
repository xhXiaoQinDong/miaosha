...
public class MiaoshaService {
	...
	public BufferedImage createVerifyCodeRegister() {
		...
		//把验证码存到redis中
		int rnd = calc(verifyCode);
		redisService.set(LoginKey.RegisterVerifyCode,"regitser",rnd);
		//输出图片
		return image;
	}
	...
	public boolean checkVerifyCodeRegister(int verifyCode) {
		Integer codeOld = redisService.get(LoginKey.RegisterVerifyCode,"regitser", Integer.class);
		if(codeOld == null || codeOld - verifyCode != 0 ) {
			return false;
		}
		redisService.delete(LoginKey.RegisterVerifyCode, "regitser");
		return true;
	}
